...
class PPBDataset(Dataset):
    def __init__(self, csv_file, tokenizer, max_length=1024):
        ...
        print(f"加载PPB数据集: {csv_file}, 样本数: {len(self.data)}")
        ...
        for idx, row in self.data.iterrows():
            ...
            total_positions += len(label_str)
            positive_positions += label_str.count('1')
            if len(label_str) > max_label_len:
                max_label_len = len(label_str)
            if len(label_str) > max_length:
                print(f"第{idx}条样本标签长度超出max_length: {len(label_str)} > {max_length}")
        print(f"总位点数: {total_positions}, 阳性位点数: {positive_positions}")
        print(f"阳性比例: {positive_positions/total_positions:.4f}")
        print(f"最大标签长度: {max_label_len}")
    ...
    def __getitem__(self, idx):
        ...
        # 如果label比序列短，用0填充；如果长，截断
        if len(labels) < len(actual_seq):
            labels = labels + [0] * (len(actual_seq) - len(labels))
        elif len(labels) > len(actual_seq):
            labels = labels[:len(actual_seq)]
        
        if len(labels) > self.max_length:
            print(f"样本{idx}标签超长，截断: {len(labels)} -> {self.max_length}")
            labels = labels[:self.max_length]
        
        encoding = self.tokenizer(
            actual_seq,  # 使用去除padding后的序列
            truncation=True,
        ...
        # 初始化binding_labels，长度与input_ids一致
        binding_labels = torch.full((input_ids.size(0),), -100, dtype=torch.long)
        ...
        # 只对实际序列位置设置标签
        seq_len = min(len(labels), len(seq_token_positions))
        for i in range(seq_len):
            token_pos = seq_token_positions[i]
            binding_labels[token_pos] = labels[i]
        # 打印部分样本调试信息
        if idx < 3 or idx % 1000 == 0:
            print(f"样本{idx}: pdb_id={pdb_id}, prot_seq_len={len(prot_seq)}, label_len={len(label_str)}, binding_labels[:10]={binding_labels[:10].tolist()}")
        return {
            'input_ids': input_ids,
            'attention_mask': attention_mask,
            'binding_labels': binding_labels,
            'mask': (binding_labels != -100).float(),
            'pdb_id': pdb_id,
            'pep_seq': pep_seq,
...
def collate_fn(batch):
    ...
    for item in batch:
        ...
        binding_labels = torch.cat([item['binding_labels'], torch.full((pad_len,), -100, dtype=torch.long)])
        mask = torch.cat([item['mask'], torch.zeros(pad_len, dtype=torch.bool)])
        
        padded_input_ids.append(input_ids)
        padded_attention_mask.append(attention_mask)
        padded_binding_labels.append(binding_labels)
        padded_mask.append(mask)
    
    return {
        'input_ids': torch.stack(padded_input_ids),
        'attention_mask': torch.stack(padded_attention_mask),
        'binding_labels': torch.stack(padded_binding_labels),
        'mask': torch.stack(padded_mask)
    }

def train_ppb(config_path, data_path, output_dir, epochs=10, batch_size=4, lr=1e-4, device='cuda'):
    # 自动生成包含参数信息的输出路径
    if output_dir is None:
        # 直接使用任务目录，不创建参数子目录
        output_dir = os.path.join("/public/home/lingwang/MoRE/Pretrain/lora", "PPB")
    
    print(f"开始PPB结合位点预测预训练")
    ...
    with open(params_txt_path, 'w', encoding='utf-8') as f:
        f.write("PPB预训练参数配置\n")
        f.write("=" * 50 + "\n")
        f.write(f"任务类型: PPB (Protein-Peptide Binding)\n")
    ...
    # 加载模型和分词器（自动检测任务名称）
    model, tokenizer = get_model(config_path, task_name="PPB")
    model = model.to(device)
    
    # 数据加载并划分训练集和验证集（PPB使用1024长度）
    full_dataset = PPBDataset(data_path, tokenizer, max_length=1024)
    ...
    # 损失函数：交叉熵损失，忽略-100位置
    def ce_loss_with_mask(logits, target, mask):
        # 直接使用CrossEntropyLoss，它会自动忽略-100
        loss = nn.CrossEntropyLoss(ignore_index=-100, reduction='mean')(logits.view(-1, logits.size(-1)), target.view(-1))
        return loss
    ...
    for epoch in range(max_epochs):
        ...
        for batch in pbar:
            ...
            binding_labels = batch['binding_labels'].to(device)
            ...
            # 计算损失
            loss = ce_loss_with_mask(logits, binding_labels, mask)
        ...
        if val_loss < best_val_loss:
            ...
            patience_counter = 0
            # 保存最佳模型
            saver = LoRAModelSaver(config_path, task_name="PPB")
    ...
    plt.xlabel('Epoch')
    plt.ylabel('Loss')
    plt.title('PPB Training Progress')
    plt.legend()
    ...
    print(f"PPB预训练完成，最佳验证损失: {best_val_loss:.6f}")

def main():
    import argparse
    parser = argparse.ArgumentParser(description='PPB结合位点预测预训练')
    parser.add_argument('--config', type=str, default=os.path.join(project_root, 'pretrain_config.json'))
    parser.add_argument('--data', type=str, default=os.path.join(project_root, '../data/PPB.csv'))
    ...
    train_ppb(
        ...
        data_path=args.data,
        output_dir=args.output,  # 如果为None，会自动生成路径
        epochs=args.epochs,
